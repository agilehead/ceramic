(function() {
    ...
    */
	...
	var Validator = function(typesManager) {
	...
	Validator.prototype.validate = function*(obj, entitySchema) {
		...
		for (fieldName in entitySchema.schema.properties) {
			...
			addValidationError(errors, fieldName, yield* this.validateField(obj, obj[fieldName], fieldName, def));
		}

		if (entitySchema.schema.required && entitySchema.schema.required.length) {
			for (_i = 0; _i < entitySchema.schema.required.length; _i++) {
				field = entitySchema.schema.required[_i];
				...
				switch (fieldDef.type) {
					...
					default:
						if (!isPrimitiveType(fieldDef.type)) {
							if (value.validate) {
								errors = errors.concat(yield* value.validate(this.typesManager));
		...
		if (error instanceof Array) {
			if (error.length > 0) {
				for (_i = 0; _i < error.length; _i++) {
					item = error[_i];
					addValidationError(list, fieldName, item);
				}
			}
			return list;
		}
