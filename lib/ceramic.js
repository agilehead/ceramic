(function() {
    ...
    */
    ...
    var TypesManager = function(options) {
        ...
        if (options) {
            if (options.fn) {
                /*
                    isModel and getDynamicEntitySchema can also be substituted by deriving from TypesManager
                */
    ...
    TypesManager.prototype.completeEntitySchema = function(def) {
        if (!def.schema) {
            def.schema = {};
        }
    ...
    TypesManager.prototype.completeVirtualEntitySchema = function*(virtualTypeDef, baseEntitySchema) {
    ...
    TypesManager.prototype.buildTypeCache = function*(entitySchemas, virtualEntitySchemasList) {
        var self = this;

        var def, type;

        //Add the base types
        entitySchemas.forEach(function(def) {
            var fullDef = self.completeEntitySchema(def);
    ...
    */
    TypesManager.prototype.getEntitySchemas = function() {
        return typeCache;
    };
    ...
    */
    TypesManager.prototype.getEntitySchema = function*(name, dynamicResolutionContext) {
        if (typeCache[name])
    ...
    */
    TypesManager.prototype.getDynamicEntitySchema = function*(name, dynamicResolutionContext) {
        throw new Error("resolveDynamicEntitySchema() method must be overridden in derived class");
    };
    ...
    */
    TypesManager.prototype.getEffectiveEntitySchema = function*(obj, entitySchema) {
    ...
    */
    TypesManager.prototype.constructModel = function*(obj, entitySchema, options) {
        var effectiveTypeDef = yield* this.getEffectiveEntitySchema(obj, entitySchema);
        var result = {};
    ...
    */
    TypesManager.prototype.updateModel = function*(target, obj, entitySchema, options) {
        for (var name in entitySchema.schema.properties) {
            ...
            if (!this.isModel(value)) {
                if (isPrimitiveType(def.type)) {
                        ...
                        if (def.type === 'array') {
                            if (def.items.entitySchema) {
                                ...
                                for (var _i = 0; _i < value.length; _i++) {
                                    var item = value[_i];
                                    arr.push(yield* this.constructModel(item, def.items.entitySchema, options));
                                }
                                value = arr;
                            }
                        }
                    ...
                    if (def.entitySchema) {
                        if (value) {
                            value = yield* this.constructModel(value, def.entitySchema, options);
                        }
                    }
                }
            }
    ...
    TypesManager.prototype.validate = function*(obj, entitySchema) {
	...
	var initted = false;
    TypesManager.prototype.init = function*(ctors, virtualEntitySchemas) {
        if (!initted) {
            yield* this.buildTypeCache(ctors, virtualEntitySchemas);
        } else {
